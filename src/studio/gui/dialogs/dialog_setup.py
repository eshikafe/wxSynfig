...
class DialogSetup(wx.Dialog):
    ...
    def __init__(self, parent, ID, title=_(APP_NAME + " Setup"), size=(500,550), pos=wx.DefaultPosition, style=wx.DEFAULT_DIALOG_STYLE,):
        wx.Dialog.__init__(self, parent, ID, title, pos, size, style)
        
        vert_sizer = wx.BoxSizer(wx.VERTICAL)
        hor_sizer = wx.BoxSizer(wx.HORIZONTAL)
        
        # Setup the lower buttons
        ...
        # Gamma
        gamma_tab = wx.Panel(notebook)
        notebook.AddPage(gamma_tab, _("Gamma"))

        vbox = wx.BoxSizer(wx.VERTICAL)

        h1 = wx.BoxSizer(wx.HORIZONTAL)
        red = wx.StaticText(gamma_tab, label="Red  ")
        h1.Add(red, 0, wx.ALL, 2)
        scale_gamma_red = FloatSlider(gamma_tab, (0.1,3.0,2.2),slidersize=(200,20),slidertextsize=(50,-1))
        h1.Add(scale_gamma_red, 1, wx.ALL, 2)

        h2 = wx.BoxSizer(wx.HORIZONTAL)
        green = wx.StaticText(gamma_tab, label="Green")
        h2.Add(green, 0, wx.ALL)
        scale_gamma_green = FloatSlider(gamma_tab, (0.1,3.0,2.2),slidersize=(200,20),slidertextsize=(50,-1))
        h2.Add(scale_gamma_green, 1, wx.ALL,2)
        
        h3 = wx.BoxSizer(wx.HORIZONTAL)
        blue = wx.StaticText(gamma_tab, label="Blue  ")
        h3.Add(blue, 0, wx.ALL)
        scale_gamma_blue = FloatSlider(gamma_tab, (0.1,3.0,2.2),slidersize=(200,20),slidertextsize=(50,-1))
        ...
        gamma_tab.SetSizer(vbox)

        # Misc
        misc_tab = wx.Panel(notebook)
        notebook.AddPage(misc_tab, _("Misc"))

        # Document
        document_tab = wx.Panel(notebook)
        notebook.AddPage(document_tab, _("Document"))

        # Render
        render_tab = wx.Panel(notebook)
        notebook.AddPage(render_tab, _("Render"))
